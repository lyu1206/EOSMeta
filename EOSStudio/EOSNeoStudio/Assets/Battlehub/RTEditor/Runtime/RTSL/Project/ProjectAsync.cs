...
namespace Battlehub.RTSL
{
    ...
    public class ProjectAsyncWithAssetLibraries<TID> : ProjectAsyncImpl<TID>, IProjectAsyncWithAssetLibraries where TID : IEquatable<TID>
    {
        ...
        public override async Task<ProjectItem> LoadImportItemsAsync(string libraryName, bool isBuiltIn, CancellationToken ct)
        {
            ...
            else
            {
                ...
                AssetBundle loadedAssetBundle = LoadedAssetBundles.Where(assetBundle => assetBundle.name == libraryName).FirstOrDefault();
                ...
                TextAsset bundleIDAsset = loadedAssetBundle.LoadAsset<TextAsset>("BundleID");
                if (assetBundleInfo == null)
                {
                    assetBundleInfo = new AssetBundleInfo();
                    assetBundleInfo.UniqueName = libraryName;

                    if (bundleIDAsset == null || string.IsNullOrEmpty(bundleIDAsset.text) || !int.TryParse(bundleIDAsset.text.Replace("\r", "").Replace("\n", ""), out int bundleID))
                    {
                    ...
                    {
                        assetBundleInfo.Ordinal = AssetLibraryInfo.BUNDLEDLIB_FIRST + bundleID;
                    }

                    m_assetLibraryIDToAssetBundleInfo.Add(assetBundleInfo.Ordinal, assetBundleInfo);
                    Debug.Log("Asset bundle " + libraryName + " has been assigned ID " + assetBundleInfo.Ordinal);
                }
            ...
            for (int i = 0; i < assetNames.Length; ++i)
            {
                ...
                if (obj is GameObject)
                {
                    ...
                    foreach (UnityObject subAsset in subAssets)
                    {
                        ...
                        {
                            string path = assetName + "/" + subAsset.name + "@@@" + subAsset.GetType().FullName;
                            if (!pathToBundleItem.TryGetValue(path, out var subItem))
                            {
                                subItem = new AssetBundleItemInfo
                                {
                                    Id = info.Identifier,
    ...
    public class ProjectAsyncImpl<TID> : IProjectAsync, IProjectEvents, IProjectState, IProjectUtils, IDisposable where TID : IEquatable<TID>
    {
        ...
        public object InstanceIDToPersistentID(UnityObject obj)
        {
            return AssetDB.CreateID(obj);
        }
                ...
                if (previews != null && previews.Length > 0)
                {
                    for (int j = 0; j < previews.Length; ++j)
                    {
                        ...
                        if (m_state.TryGetProjectItemFromCache(id, out projectItem))
                        {
                            ...
                            {
                                Debug.LogErrorFormat("asset item {0} with wrong parent selected. Expected parent {1}. Actual parent {2}", projectItem, folder.ToString(), projectItem.Parent.ToString());
                                continue;
                            }
        ...
        protected async Task<ProjectItem[]> SaveAsync(ProjectItem[] existingAssetItems, ProjectItem[] folders, byte[][] previewData, object[] objects, string[] nameOverrides, CancellationToken ct)
        {
            ...
            for (int objIndex = 0; objIndex < objects.Length; ++objIndex)
            {
                ...
                int assetLibraryID = m_assetDB.GetAssetLibraryID(m_storage.GetID(assetItem));
                if (assetLibraryID != -1)
                {
                ...
                {
                    assetLibraryID = m_assetDB.GetAssetLibraryID(id);
                    if (assetLibraryID != -1)
                    {
                        assetLibraryIds.Add(assetLibraryID);
                    }
        ...
        protected void Remove(ProjectItem projectItem)
        {
            ...
            if (m_state.LoadedScene == projectItem)
            {
                m_state.LoadedScene = null;
            }
        }
